...
\begin{enumerate}
    ...
    \item Move the particle according to the velocity field. Either by computing $\textbf{x}_p^{new} = \triangle t \u_p$ to $\textbf{x}_p$, or, for better precision, using a higher-order Runge-Kutta integration.
...
With the boundary conditions satisfied, the equation become a system of $N$ linear equations with $N$ variables, with $N$ being the amount of fluid cells. Solving this equations hence results in a discrete representation of the pressure field $p$, which satisfies 
$$
\nabla \cdot
 ( \u_{[n+1]}^{compressible} - \triangle t \frac{\nabla p}{\rho} ) = 0
$$
...
\begin{algorithm}[H]
    ...
    \ForEach{particle $p$}{
        ...
        $\textbf{x}_p := \textbf{x}_p + \u_p \triangle t$ (or using a higher order Runge-Kutta)\;
    }
    \ForEach{grid cell at location $(x,y,z)$}{
        Find all particles within a radius of $\triangle x$\;
        Compute $\u_{[n+1]}^{advected}$, as an average of the $\u_p$ carried by nearby particles\;
    }
...
\begin{algorithm}[H]
    ...
    \ForEach{grid cell at location $(x,y,z)$}{
        Find all particles within a radius of $\triangle x$\;
        Compute $\u_{[n+1]}^{advected}$, as an average of the $\u_p$ carried by nearby particles\;
        Compute $\alpha_{[n+1]}^{advected}$, as an average of the $\alpha _p$ carried by nearby particles\;
    }
    Apply external forces, $\u_{[n+1]}^{compressible} =  FixSolidBoundary(u_{[n+1]}^{advected} + \triangle t \textbf{g})$\;
    Construct and solve the Poisson pressure equation, obtain pressure $p$\;
    Compute $\u_{[n+1]} = \u_{[n+1]}^{compressible} - \triangle t \dfrac{\nabla p}{\rho}$\;
