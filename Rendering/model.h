    ...
    Model(std::string const & path, glm::vec3 scale = glm::vec3(1,1,1),glm::vec3 translate = glm::vec3(0,0,0),bool gamma = false ) : gammaCorrection(gamma)
    {
        ...
        for(Mesh&m:meshes){
            for (int i = 0; i < m.indices.size(); ++i) {
                Triangles.push_back(m.vertices[m.indices[i]]);
            }
        }
    }

    // Draws the SceneModel, and thus all its meshes
    void Draw(Shader& shader,bool addModel=true)
    {
    ...
    std::vector<Texture> loadMaterialTextures(aiMaterial* mat, aiTextureType type, std::string typeName)
    {
        ...
        for(GLuint i = 0; i < mat->GetTextureCount(type); i++)
        {
            ...
            {   // If texture hasn't been loaded already, load it
                ...
                this->textures_loaded.push_back(texture);  // Store it as texture loaded for entire SceneModel, to ensure we won't unnecesery load duplicate textures.
            }
        }
        return textures;
    }
...
GLint TextureFromFile(const char* path, std::string directory, bool gamma)
{
    ...
    unsigned char* image = stbi_load(filename.c_str(), &width, &height, &nrChannels, 3);
